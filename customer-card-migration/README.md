...
3. **상태 충돌을 우선순위로 덮어쓰지 마세요.** 충돌 7건은 `mou.status = "상태 충돌·담당자 확인 필요"` 이고,
   우선순위만 적용했을 때의 값은 `mou.statusCandidate` 에 따로 있습니다.
4. **날짜를 지어내지 마세요.** 날짜로 해석되지 않는 원문(`조율중`, `예정`, `X`)은 `mou.rawLabels` 에 원문 그대로 있습니다.
5. **경과일은 다시 계산하세요.** `mou.elapsedDays` 는 2026-09-02 기준 스냅샷입니다.
6. **`id` 는 영구 키가 아닙니다.** 조인은 `companyCode` → `businessNumber` → 정규화 업체명 순으로 하세요
   (`CUSTOMER-CARD-MAPPING.md` §9).
7. **개인정보** — JSON 에는 대표자·연락처·이메일·주소·사업자번호 원본값이 들어 있습니다.
