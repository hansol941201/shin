...
|---|---|
| MOU 체결 완료 | 219 |
| MOU 체결 완료·체결일 미확인 | 60 |
| MOU 진행 중 | 44 |
| 허들·보류 | 17 |
| 종결 | 0 |
| 기존 협력업체·MOU 상태 확인 필요 | 10 |
| 상태 충돌·담당자 확인 필요 | 1 |
| **합계** | **351** |
...
1. **기존 협력업체 ≠ MOU 체결.** `isExistingPartner` 와 `mou.status` 는 **별개 속성**입니다.
   협력업체 리스트에 있다는 이유만으로 체결로 추정하지 않았습니다.
   레거시 관리 엑셀의 “전체 협약업체” 명부에서 **문서 근거가 확인된 61개사**만
   `MOU 체결 완료·체결일 미확인` 으로 옮겼고(근거는 `mou.evidence`), 근거가 없는 10개사는
   그대로 “상태 확인 필요”로 두었습니다. **체결일은 그 엑셀에도 없어 채우지 않았습니다.**
...
`기존 협력업체·MOU 상태 확인 필요` 71개사 중 **61개사**가 협약 명부 근거로
`MOU 체결 완료·체결일 미확인` 으로 이전됐고, 엑셀에만 있던 **1개사**(체결일 포함)가 추가됐습니다.
다만 **그 엑셀에도 61개사의 체결일은 없습니다** — 체결일이 적힌 시트는 신규 MOU 프로세스
CRM 두 개뿐이고, 등급·매출 시트에는 체결일 컬럼 자체가 없습니다. 자세한 내용은
`DATA-VALIDATION-REPORT.md` §3-1 을 보세요.
