#!/usr/bin/env node
/**
 * partner-dashboard → 고객관리카드 마이그레이션 데이터 빌더
 *
 * 입력 : pourservice/partner-dashboard 의 index.html (읽기 전용)
 *        - 내장 시드 `const DATA = {...}`
 *        - 내장 상수 CHANGELOG / CHECKLIST_DATA / STAGE_NAMES / recalcStage()
 * 출력 : customer-card-migration/companies-integrated.json
 *        customer-card-migration/DATA-VALIDATION-REPORT.md
 *
 * 원칙: 값을 추측하지 않는다. 확인 불가한 값은 null 로 두고 사유를 validation.messages 에 남긴다.
 */
'use strict';
const fs = require('fs');
...
const seedMatch = html.match(/^const DATA = (\{.*\});\s*$/m);
if (!seedMatch) throw new Error('index.html 에서 const DATA 시드를 찾지 못했습니다.');
const DATA = JSON.parse(seedMatch[1]);

/** index.html 안의 `const NAME = [...];` 블록을 그대로 평가해서 가져온다. */
...
const CHANGELOG = extractArrayConst('CHANGELOG') || [];
const CHECKLIST_DATA = extractArrayConst('CHECKLIST_DATA') || [];
...
const notCollected = [
  { item: 'Firebase 실시간 동기화 데이터 (사용자가 화면에서 수정·추가한 최신분)',
    reason: '원본 사이트는 정적 시드 위에 Firebase Realtime Database(pour-partner-dashboard…asia-southeast1)와 브라우저 localStorage 값을 덮어써서 표시합니다. 해당 도메인은 이 작업 환경의 네트워크 정책에서 차단되어 접근할 수 없었고, 비공개 시스템이므로 인증 접근도 시도하지 않았습니다.',
    impact: '본 데이터는 GitHub Pages 에 배포된 공개 정적 기준값(최종 배포 2026-07-23)입니다. 운영 중 수정분이 있으면 반영되지 않습니다.' },
  { item: '허들·보류 “액션 결정” (재접근 / 종결 / 보류 유지 / 재발송 / 확인 필요)',
    reason: '원본에서 localStorage 키 hurdleActions 에만 저장되며 배포 데이터에 포함되지 않습니다.',
...
// 콘솔 요약
console.log(JSON.stringify({ summary: { ...summary, menuCounts: undefined }, menuCounts }, null, 1));
console.log('\n--- statusConflict rows ---');
companies.filter(x => x.validation.statusConflict).forEach(x => console.log(' ', x.companyName, '::', x.validation.messages.filter(m => m.type === 'statusConflict').map(m => m.message).join(' ; ')));
console.log('\n--- dateError rows ---');
companies.filter(x => x.validation.dateError).forEach(x => console.log(' ', x.companyName, '::', x.validation.messages.filter(m => m.type === 'dateError').map(m => m.message).join(' ; ')));
console.log('\n--- duplicate suspects ---');
companies.filter(x => x.validation.possibleDuplicate).forEach(x => console.log(' ', x.companyName, '::', x.validation.messages.filter(m => m.type === 'possibleDuplicate').map(m => m.message).join(' ; ')));
