...
### Windows - 바탕화면 아이콘 (권장, 실사용 방식)

저장소 최상위(루트)의 `설치.bat`을 **처음 한 번만** 더블클릭하세요. 바탕화면에
"팀장 일정" 아이콘이 자동으로 생성됩니다. 이후에는 그 아이콘만 더블클릭하면 됩니다.

- **검은 터미널/CMD 창이 전혀 보이지 않습니다.** 매번 클릭할 때마다 최신
  코드로 production build를 만들고(개발 서버가 아님), 그 결과를 숨겨진
  프로세스로 서빙한 뒤, Edge를 `--app` 모드(주소창/탭바 없는 프로그램 창처럼
  보이는 모드, 창 제목은 "팀장 일정")로 열어줍니다. Edge를 찾지 못하면 Chrome,
  그마저 없으면 기본 브라우저 탭으로 엽니다.
- **Google 연결도 매번 다시 누를 필요가 없습니다.** 최초 1회 "Google 캘린더
  연결"로 로그인/권한 허용을 마치면, 이후에는 앱을 껐다 켤 때마다 자동으로
  조용히 재연결을 시도합니다(사용자 개입 없음). Google 쪽 로그인이 아예 끊긴
  경우에만 "Google 다시 연결" 버튼이 나타납니다. 선택해둔 팀장 캘린더도
  그대로 기억됩니다.
...
- `node_modules`가 없으면 자동 설치, 있으면 건너뜁니다. 포트는 항상 `5173`으로
  ...
  전환하지 않습니다). 5173을 이미 다른 프로그램이 쓰고 있으면, **응답 여부가
  아니라 "지금 이 폴더에서 실행된 프로세스인가"를 먼저 확인**합니다(이전에
  압축을 풀어둔 구버전 폴더의 서버가 계속 떠 있으면 새 코드가 있는데도 옛
  화면이 계속 보이는 문제를 막기 위함입니다):
  - 지금 이 폴더에서 실행된 서버가 정상 응답 중이면 → 새로 켜지 않고 그대로 사용
  - 지금 이 폴더에서 실행된 서버인데 응답이 없으면(멈춤) → 자동으로 정리하고
    다시 시작
  - **다른 폴더**에 있는 팀장 일정 서버(예: 이전에 풀어둔 구버전 ZIP)라면 →
    응답 여부와 상관없이 종료하고 지금 이 폴더의 최신 코드로 다시 시작
  - 팀장 일정과 전혀 무관한 다른 프로그램이라면 → 임의로 종료하지 않고,
    프로그램 이름과 PID를 알려주는 한글 안내와 함께 멈춥니다
  - 새 버전을 받아 폴더를 새로 풀었는데 화면이 안 바뀌는 것 같으면, 브라우저
    탭도 새로고침(`Ctrl+Shift+R`)해서 캐시된 옛 화면이 아닌지 확인하세요.
- 저장소를 다른 위치로 옮기면 `설치.bat`을 다시 실행해 바로가기를 재생성하세요.
- 오류가 나면 한글 안내 메시지 창(팝업)으로 원인을 보여줍니다.
- 서버를 끄려면 `team-schedule-app\종료.bat`을 더블클릭하세요.

### Windows - 폴더에서 직접 실행 (대안, 문제 진단용)

`team-schedule-app` 폴더 안의 `실행.bat`을 더블클릭하면 검은 CMD 창에 로그가
그대로 보이면서 production build → 정적 서버 실행 → 앱 창 자동 오픈까지
동일하게 진행됩니다(개발 서버가 아니라 바탕화면 아이콘과 같은 방식이며, 다만
콘솔 창을 남겨서 문제가 생겼을 때 원인을 눈으로 볼 수 있게 한 버전입니다).
창을 닫으면 서버도 함께 종료됩니다. 같은 폴더의 `종료.bat`으로도 서버를
종료할 수 있습니다.
...
### 개발자용: 코드를 직접 고칠 때만 (Vite 개발 서버)

일반 사용자는 이 방법을 쓸 필요가 없습니다. 코드를 수정하며 HMR(즉시 반영)로
확인하고 싶을 때만 사용하세요.
...
브라우저에서 `http://localhost:5173` 접속. 실사용(바탕화면 아이콘/실행.bat)은
항상 `npm run build` + `npm run preview`(production build를 정적으로 서빙)만
사용하며, 개발 서버(`npm run dev`)는 절대 실행하지 않습니다.
...
발급받은 클라이언트 ID를 붙여넣고 Enter만 하면 됩니다. 그 이후는 전부
자동입니다.

- 폴더 이름이 아니라 **실제로 `package.json`과 `vite.config.*`가 함께 있는
  폴더**를 찾아서 그 폴더 바로 아래에 `team-schedule-app\.env.local`을
  저장합니다(Git에는 포함되지 않음).
- 바탕화면/저장소 루트에 잘못된 이름(`team-schedule-app.env.local`)이나
  숨겨진 확장자 실수(`.env.local.txt`)로 만들어진 예전 파일이 있으면 자동으로
  찾아서 정리합니다.
- 저장한 뒤 실제 파일을 다시 읽어 값이 정확히 일치하고
  `.apps.googleusercontent.com`으로 끝나는지 검증합니다.
- 다른 폴더에 남아있던 예전 버전의 서버까지 포함해 5173 포트를 쓰던 서버를
  전부 종료하고, **이 스크립트가 직접 서버를 새로 켜서 브라우저까지
  자동으로 엽니다.** 바탕화면 아이콘을 따로 다시 실행할 필요가 없습니다.

즉 `구글연동설정.bat` 실행 → 클라이언트 ID 붙여넣기 → 끝. 잠시 후 브라우저가
자동으로 열리고 헤더에 **"Google 캘린더 연결"** 버튼이 나타납니다.
