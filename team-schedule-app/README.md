...
이 폴더(`team-schedule-app`) 안의 `실행.bat`을 더블클릭하면 됩니다.

- `node_modules`가 없으면 자동으로 `npm install`을 실행하고, 이미 있으면 건너뜁니다.
- Vite 개발 서버를 시작하고, 서버가 준비되면 기본 브라우저가 자동으로 열립니다.
- 5173번 포트가 이미 사용 중이면 5174, 5175 … 순서로 자동으로 빈 포트를 찾습니다.
- 창을 닫으면 서버도 함께 종료됩니다. 창을 닫지 않고 종료하려면 같은 폴더의
  `종료.bat`을 더블클릭하세요(실행 중인 서버 포트를 찾아 종료합니다).
- Node.js가 설치되어 있지 않거나 오류가 발생하면 창이 바로 닫히지 않고 오류
  메시지를 보여준 뒤 대기합니다. (Node.js가 없다면 https://nodejs.org 에서
  LTS 버전을 먼저 설치해주세요.)
- Google/Firebase 설정 없이도 데모 모드로 정상 동작합니다.
