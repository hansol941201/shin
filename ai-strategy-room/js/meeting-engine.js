...
/**
 * [우리 팀 실행 방향] = "30초 요약" — 회의 결과를 "10대가 읽어도 30초 안에
 * 뭘 해야 할지 아는" 아주 쉬운 말로 한 번 더 번역한 요약을 상세 보고서보다
 * 먼저 배치한다. 전문가 회의 자체(7명/반박/가드레일/심사)와 상세 분석의
 * 깊이는 전혀 줄이지 않는다 — 그 결과물을 화면 맨 위에 쉽게 번역해서 얹는
 * 것만 추가한다. "## 우리 팀 실행 방향"과 7개 "### N. 제목"은 프로그램이
 * ReportBuilder.splitActionPlan / QualityGuardrails.report에서 그대로 찾아
 * 파싱하므로 절대 바꾸거나 요약하지 마라(상세 보고서의 소제목과 달리 이 부분만
 * 문구를 고정한다).
 */
const ACTION_PLAN_GUIDE = `상세 분석 보고서보다 먼저, 화면 맨 위에 [우리 팀 실행 방향] 요약을 아주 쉬운 말로 작성한다. 이 요약을 읽는 사람은 회의 내용을 전혀 모른다고 생각하고, 10대가 읽어도 30초 안에 "그래서 우리가 뭘 하면 되는지" 알 수 있게 써라.

이 요약 안에서는 지켜야 할 규칙이 있다:
- 전문용어·업계 용어·긴 문장을 쓰지 마라(전문용어와 근거는 상세 분석 보고서에서만 마음껏 써라).
- 부득이하게 어려운 말을 써야 한다면 반드시 괄호 안에 쉬운 뜻을 같이 적어라. 예: "이탈률(다시 안 쓰고 나가는 비율)".
- 한 문장에는 하나의 내용만 담아라. 여러 내용을 접속사로 길게 잇지 마라.
- "홍보 강화", "교육 강화", "사용자 경험 개선" 같은 추상적인 표현은 절대 쓰지 마라. 실제로 무엇을 만들고 무엇을 확인할지 구체적으로 써라.

"##"·"###" 제목 문구는 절대 바꾸지 말고 그대로 써라.

## 우리 팀 실행 방향

### 1. 그래서 결론은?
가장 중요한 결론 하나만 쉬운 말 1~2문장으로 크게 쓴다. 예: "기존 방식을 바로 없애지 말고, 일부 업체부터 새 방식을 써보게 한 뒤 반응을 보고 넓힙니다."

### 2. 우리가 준비할 것
실제로 만들거나 확보해야 하는 것만 markdown 표로 쓴다. 각 이유는 전문용어 없이 한 문장으로.
| 준비할 것 | 왜 필요한지 |
|---|---|
| ... | ... |

### 3. 이렇게 하면 됨
전체 과정을 5단계 정도의 화살표 흐름 한 줄로 쓴다. 안건에 맞게 단계 이름은 바꿔도 되지만 반드시 화살표(→)로 이어라. 예: ① 준비 → ② 몇 곳 먼저 실행 → ③ 반응 확인 → ④ 문제 수정 → ⑤ 전체 확대

### 4. 지금 당장 할 일
오늘 또는 회의 직후 바로 시작할 수 있는 일만 최대 5개, "□ " 체크리스트로 쓴다.

### 5. 누가 뭘 하지?
담당과 할 일만 담은 아주 간단한 표로 쓴다(실제 담당자를 모르면 운영/기획/고객응대/데이터/개발/책임자 같은 역할로 쓴다).
| 담당 | 할 일 |
|---|---|
| ... | ... |

### 6. 하지 말아야 할 것
전문가 회의에서 위험하거나 효과가 낮다고 판단한 행동만 "X " 표시로 최대 3~5개 쓴다.

### 7. 성공했는지 어떻게 알지?
숫자를 억지로 만들지 말고, 무엇을 보면 성공인지 알 수 있는 단계를 화살표(↓)로 이어서 쓴다. 마지막 단계는 반드시 별도의 화살표(↓)로 구분해서 데이터 현황만 짧게 쓴다. 예: 설치했나? ↓ 실제로 사용했나? ↓ 현장을 등록했나? ↓ 다시 사용했나? ↓ 아직 데이터 없음 → 2주 측정 후 기준값 설정. 실제 데이터가 있으면 그 마지막 단계 자리에 실제 수치를 써라. 근거 없는 목표 퍼센트는 절대 만들지 마라.

시공사가 관련된 안건이라면 [2. 우리가 준비할 것]과 [3. 이렇게 하면 됨]에 반드시 시공사 입장에서 "이걸 왜 해야 하는가", "귀찮아하지 않을까", "실제로 행동할 이유가 있는가"를 검토한 결과를 반영하라.

이 요약을 쓴 다음에 상세 분석 보고서를 이어서 작성한다(전문용어·데이터·근거·반박 내용은 거기서는 마음껏 써도 된다). 전문가 7명의 토론 깊이나 반박 내용을 줄이라는 뜻이 아니다 — 어려운 회의 결과는 그대로 유지한 채, 처음 보는 사람도 이해할 수 있도록 한 번 더 쉬운 말로 번역해서 요약을 얹는 것이다.`;
