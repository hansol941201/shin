# -*- coding: utf-8 -*-
"""
자동화 자료취합 - legacy v1 파이프라인(보존용, 활성 경로 아님).
고정 페이지 템플릿 + 카테고리당 소수 대표 사진 방식. 실제 서비스에는 app.engine.pipeline.run_pipeline_v2 를 사용한다.
"""
import argparse
import datetime
import os
...
from app.legacy import selector_v1 as cs
...
from app.legacy.ppt_generator.generator import generate_pptx
from app.ppt_parser.parser import make_workcopy, parse_presentation
from app.legacy.slide_planner.planner import build_content_plan, plan_slides
...
def run_pipeline_v1(apartment_name: str, work_type: str, input_paths: List[str], output_dir: str,
    ...
    # 1. PPT 분석
    _report(progress_cb, STAGES_V1[0])
    ...
    # 2. 이미지 추출
    _report(progress_cb, STAGES_V1[1])
    all_images = []
    ...
    _report(progress_cb, STAGES_V1[2])
    ...
    _report(progress_cb, STAGES_V1[3])
    ...
    _report(progress_cb, STAGES_V1[4])
    ...
    _report(progress_cb, STAGES_V1[5])
    used_ids = set()
    ...
    _report(progress_cb, STAGES_V1[6])
    ...
    # 8. PPTX 생성
    _report(progress_cb, STAGES_V1[7])
    ...
    # 9. 검수
    _report(progress_cb, STAGES_V1[8])
    ...
    _report(progress_cb, STAGES_V1[9])
